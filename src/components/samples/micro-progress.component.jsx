...
import JSCLabel from '../shared/jsc-label.component.jsx';
...
export default class MicroProgressComponent extends React.Component {
    ...
    componentDidMount() {
        this.showProgress((config) => { this.setState({ labelConfig1: config }); }, () => {
			this.showProgress((config) => { this.setState({ labelConfig2: config }); }, () => {
				this.showProgress((config) => { this.setState({ labelConfig3: config }); }, () => {
                    this.setState({ finalLabelConfig: '<span style="font-weight:bold; font-size:20px">The installation was successful!</span><br>' });
				});
			});
		});
    }

    showProgress(configFn, cb) {
    ...
    render() {
		return (
			<div>
				<JSCLabel config={this.state.labelConfig1} className={'progress-label'}></JSCLabel>
                <JSCLabel config={this.state.labelConfig2} className={'progress-label'}></JSCLabel>
                <JSCLabel config={this.state.labelConfig3} className={'progress-label'}></JSCLabel>
                <JSCLabel config={this.state.finalLabelConfig} className={'progress-label'}></JSCLabel>
			</div>
		);
	}
}
